...
package com.ajjpj.afoundation.concurrent.jdk.j9new;
...
public class ForkJoinPool extends AbstractExecutorService {
    ...
    private WorkQueue findNonEmptyStealQueue() {
        ...
        int r = ThreadLocalRandomHelper.nextSecondarySeed();
        if ((ws = workQueues) != null && (m = ws.length - 1) >= 0) {
            for (int origin = r & m, k = origin, oldSum = 0, checkSum = 0;;) {
                WorkQueue q; int b;
                if ((q = ws[k]) != null) {
    ...
    private void externalSubmit(ForkJoinTask<?> task) {
        ...
        if ((r = ThreadLocalRandomHelper.getProbe()) == 0) {
            ThreadLocalRandomHelper.localInit();
            r = ThreadLocalRandomHelper.getProbe();
        }
        for (;;) {
            ...
            if (move)
                r = ThreadLocalRandomHelper.advanceProbe(r);
        }
    }
