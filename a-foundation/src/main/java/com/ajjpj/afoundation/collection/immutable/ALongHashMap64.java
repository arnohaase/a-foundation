 ...
 * @author arno
 */
public class ALongHashMap64<V> implements AMap<Long,V>, Serializable {
    ...
    private static final ALongHashMap64 EMPTY = new ALongHashMap64 ();
    ...
    public static <V> ALongHashMap64<V> empty() {
        return EMPTY;
    }

    public static <V> ALongHashMap64<V> fromJavaUtilMap(Map<? extends Number,V> map) {
        ALongHashMap64<V> result = empty ();
     ...
     */
    public static <V> ALongHashMap64<V> fromKeysAndValues(Iterable<? extends Number> keys, Iterable<V> values) {
        final Iterator<? extends Number> ki = keys.iterator();
        final Iterator<V> vi = values.iterator();

        ALongHashMap64<V> result = ALongHashMap64.empty ();
    ...
    public static <K extends Number, V, E extends Exception> ALongHashMap64<V> fromKeysAndFunction(Iterable<K> keys, AFunction1<? super K, ? extends V, E> f) throws E {
    ...
    @Override public ALongHashMap64<V> updated (Long key, V value) {
        return updated (key.longValue (), value);
    }
    public ALongHashMap64<V> updated (long key, V value) {
        return doUpdated(key, computeHash(key), 0, value);
    }

    @Override public ALongHashMap64<V> removed (Long key) {
        return removed (key.longValue ());
    }
    public ALongHashMap64<V> removed (long key) {
    ...
    ALongHashMap64<V> doUpdated(long key, long hash, int level, V value) {
        return new LongHashMap1<> (key, hash, value);
    }

    ALongHashMap64<V> doRemoved(long key, long hash, int level) {
        return this;
    }
...
//        return key;
    ...
    @SuppressWarnings("unchecked")
    private static <K,V> ALongHashMap64<V>[] createArray(int size) {
        return new ALongHashMap64[size];
    }
     ...
     */
    private static<K,V> LongHashTrieMap<V> mergeLeafMaps (long hash0, ALongHashMap64<V> elem0, long hash1, ALongHashMap64<V> elem1, int level, int size) {
        final int index0 = (int) ((hash0 >>> level) & 0x3f);
        final int index1 = (int) ((hash1 >>> level) & 0x3f);
        if(index0 != index1) {
            final long bitmap = (1L << index0) | (1L << index1);
            final ALongHashMap64<V>[] elems = createArray(2);
            if(index0 < index1) {
                elems[0] = elem0;
                elems[1] = elem1;
            }
            ...
            final ALongHashMap64<V>[] elems = createArray(1);
    ...
    static class LongHashMap1<V> extends ALongHashMap64<V> {
        ...
        @Override ALongHashMap64<V> doUpdated(long key, long hash, int level, V value) {
            if (key == this.key) {
                if(this.value == value) {
                    return this;
                }
                ...
                final ALongHashMap64<V> that = new LongHashMap1<>(key, hash, value);
                return mergeLeafMaps(this.hash, this, hash, that, level, 2);
            }
        }

        @Override ALongHashMap64<V> doRemoved(long key, long hash, int level) {
            if (key == this.key) {
                return empty();
            }
            else {
    ...
    static class LongHashTrieMap<V> extends ALongHashMap64<V> {
        final long bitmap;
        final ALongHashMap64<V>[] elems;
        final int size;

        LongHashTrieMap (long bitmap, ALongHashMap64<V>[] elems, int size) {
            this.bitmap = bitmap;
            this.elems = elems;
            this.size = size;
        }
        ...
        @Override ALongHashMap64<V> doUpdated(long key, long hash, int level, V value) {
            ...
            if ((bitmap & mask) != 0) {
                final ALongHashMap64<V> sub = elems[offset];

                final ALongHashMap64<V> subNew = sub.doUpdated(key, hash, level + LEVEL_INCREMENT, value);
                if(subNew == sub) {
                    return this;
                }
                else {
                    final ALongHashMap64<V>[] elemsNew = createArray(elems.length);
                ...
                final ALongHashMap64<V>[] elemsNew = createArray(elems.length + 1);
        ...
        @Override ALongHashMap64<V> doRemoved (long key, long hash, int level) {
            ...
            if ((bitmap & mask) != 0) {
                final ALongHashMap64<V> sub = elems[offset];
                final ALongHashMap64<V> subNew = sub.doRemoved(key, hash, level + LEVEL_INCREMENT);
