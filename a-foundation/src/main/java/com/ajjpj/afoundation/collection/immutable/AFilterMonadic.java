...
/**
 * This interface represents something like a collection, but with weaker requirements than a full-blown collection. More
 *  specifically, it supports
 *
 *
 * @author arno
 */
public interface AFilterMonadic<T> extends AMonadicOps<T>, Iterable<T> {
    /**
     * Filters this collection's elements, this method returns a new collection comprised of only those elements that match
     *  a given predicate.
     */
    @Override <E extends Exception> AFilterMonadic<T> filter(APredicate<? super T, E> pred) throws E;

    /**
     * Turns a collection of collections into a 'flat' collection, removing one layer of collections.
     */
    @Override <X> AFilterMonadic<X> flatten();
     ...
     */
    @Override <X,E extends Exception> AFilterMonadic<X> map(AFunction1<? super T, ? extends X, E> f) throws E;
     ...
     */
    @Override <X, E extends Exception> AFilterMonadic<X> flatMap(AFunction1<? super T, ? extends Iterable<X>, E> f) throws E;
